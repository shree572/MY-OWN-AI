# VectorDB — Build a Vector Database from Scratch in JavaScript

A fully working **Vector Database** built from scratch in JavaScript (Node.js) with a web UI.  
...
| **JavaScript Implementation** | Pure JavaScript with no external dependencies, uses built-in HTTP server |
...
### Step 5 — Run the VectorDB Server
...
The server will start immediately on port 8080.

> **Troubleshooting:**
> - `node: command not found` → Node.js not installed or not in PATH, redo Step 1
> - `Error: listen EADDRINUSE :8080` → Port 8080 already in use, kill the process or use different port
...
```
VectorDB/
├── main.js         ← JavaScript backend (HNSW, KD-Tree, BruteForce, REST API, RAG)
...
| `node: command not found` | Add Node.js to PATH or reinstall from nodejs.org |
...
Then edit [main.js](main.js) line where `genModel` is set:
```javascript
const genModel = "llama3.2:1b";   // change this
```
